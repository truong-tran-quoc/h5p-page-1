...
getAjaxUrl = function (action, parameters) {
  var url = "/api/LMS/Content/Courseware/" + action;
 ...
 * then runs the callback with the semantics as an argument.
 *
...
ns.loadLibrary = function (libraryName, callback) {
  switch (ns.libraryCache[libraryName]) {
    ...
    case undefined:
      ...
      var url = getAjaxUrl('getLibrarySemantics', library);
...
ns.getUrl = function (action, parameters) {
  var url = 'http://localhost:63342/H5P-editor/' + action;
  if (parameters !== undefined) {
    for (var key in parameters) {
      url += '/' + parameters[key];
    }
